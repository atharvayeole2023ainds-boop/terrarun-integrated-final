...
export const GameProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (token) {
        try {
          const resp = await fetch(`${process.env.REACT_APP_API_URL || ''}/api/auth/me`, {
          ...
          if (resp.ok) {
            ...
            if (payload?.data?.user) {
              ...
              if (payload.data.user.stats) {
                setStats((s) => ({ ...s, ...payload.data.user.stats }));
              }
              localStorage.setItem('terra_session', JSON.stringify(payload.data.user));
            }
          } else {
  ...
  const login = (userData: UserProfile) => {
    setUser(userData);
    localStorage.setItem('terra_session', JSON.stringify(userData));
    showToast(`Welcome back, Agent ${userData.username}`, 'info');
  };
